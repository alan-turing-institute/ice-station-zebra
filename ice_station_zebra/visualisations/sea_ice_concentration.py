...
def video_sic_comparison(
    ...
    if targets.shape != predictions.shape:
        raise ValueError(
            "The target and prediction sequences must have the same shape."
        )
    if len(targets) != len(predictions) or len(targets) != len(dates):
        raise ValueError(
            "The target, prediction, and date sequences must have the same length."
        )
    ...
    axs[1].contourf(initial_prediction, levels=z_range, cmap="viridis")
    ...
    plt.colorbar(
        image1, ax=axs, orientation="vertical", ticks=np.linspace(0, 1, 11), shrink=0.8
    )
    ...
    def animate(frame_idx: int):
        ...
        # Update main title with current date
        date_string = dates[frame_idx].strftime(r"%Y-%m-%d %H:%M")
        fig.suptitle(f"Sea Ice Concentration - {date_string}", fontsize=14)
    ...
    try:
        ...
        writer: Union[animation.PillowWriter, animation.FFMpegWriter]
        if format.lower() == "gif":
